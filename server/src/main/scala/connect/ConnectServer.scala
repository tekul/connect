...
import crypto.sign.{RsaVerifier, RsaSigner}
...
object OpenID extends OpenIDProvider with Logger {
  ...
  val signer = RsaSigner(n, BigInt("2494399290019089944481173041331303044590956731215317985557340303667891408969108902" +
  ...
  val verifier = RsaVerifier(n, 65537)
  ...
  val invalidToken: JValue = ("error" -> "invalid_id_token")
}
